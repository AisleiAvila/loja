...
export class AdminPageComponent implements OnInit {
  ...
  protected readonly totalProductPages = computed(() => Math.max(1, Math.ceil(this.filteredProducts().length / this.productPageSize)));
  ...
  protected saveProduct(): void {
    ...
    request.subscribe({
      ...
      error: (error) => this.feedback.set(error.error?.message || 'Não foi possível guardar o produto.')
    });
  }
  ...
  protected deleteSelectedProduct(): void {
    ...
    this.apiService.deleteProduct(token, product.id).subscribe({
      ...
      error: (error) => this.feedback.set(error.error?.message || 'Não foi possível apagar o produto.')
    });
  }
